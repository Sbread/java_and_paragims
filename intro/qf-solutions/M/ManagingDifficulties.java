...
public class ManagingDifficulties {
    ...
    public static void main(String[] args) {
        ...
        try {
            int t = parseInt(scanner.next());
            for (int i = 0; i < t; i++) {
                int n = parseInt(scanner.next());
                int ans = 0;
                int[] difficult = new int[n];
                for (int j = 0; j < n; j++) {
                    difficult[j] = parseInt(scanner.next());
                }
                Map<Integer, Integer> interimAns = new HashMap<>();
                for (int k = n - 1; k > 0; k--) {
                    for (int j = 0; j < k; j++) {
                        ans += interimAns.getOrDefault(2 * difficult[k] - difficult[j], 0);
                    }
                    interimAns.put(difficult[k], interimAns.getOrDefault(difficult[k], 0) + 1);
                }
